...
public class FixedTabbarView: UIView{
    ...
    func setTitlesAndControllers(titles: Array<String>) {
    ...
    var tabbarShowView = TabbarShowView()
    ///当前view移动之后
    typealias MoveView = (_ lastIndex: Int,_ nowIndex: Int)->Void
    var moveView: MoveView?
    ///首次显示该View
    typealias MoveViewIsFirst = (_ firstIndex: Int)->Void
    var moveViewIsFirst: MoveViewIsFirst?
    ...
    ///移动
    func moveBtn(index: Int) {//根据选中的模块去判断scroller需要滚动到哪里
        if nowIndex == index {
            return
        }
    ...
    ///设置选中时的文字颜色
    func setTitleSelectColor(color: UIColor){
        for btn in tabbarBtnArr {
            if btn.tag == nowIndex {
                btn.btnTitleLabel.font = UIFont(name: "Helvetica-Bold", size: 16)//加粗
                btn.btnTitleLabel.textColor = color
    ...
    ///设置禁止拖动
    func setNotScroll(type: Bool) {
        tabbarShowView.isScrollEnabled = type
    }
    ///设置行颜色
    func setRowColor(color: UIColor){
...
internal class TabbarShowView: UIScrollView {//主页面上的scroll，里面装每个不同的页面
